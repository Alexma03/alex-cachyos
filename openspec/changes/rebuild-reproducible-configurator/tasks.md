...
- [x] Finalize `openspec/config.yaml` transition state: `bash -n …` + profile JSON parse + `go test ./...` + `go run ./tools/sync-assets --check` all retained and passing (staged-migration: both runners continuously validated). <!-- sdd-owner: implementation -->
- [x] Docs finalized in English with quick-path + checklist structure: `docs/go-migration.md`, `docs/catalog.md`, `docs/receipts.md`, `docs/rollback.md` (pi-reproduction dropped per Pi scope removal). <!-- sdd-owner: implementation -->
- [x] Cutover evidence report `docs/cutover-evidence.md`: fixture evidence vs hardware convergence kept distinct; hardware checks listed as gated on the declared integration target only; explicit statement that this change deletes no Bash files and cutover is a separate future proposal requiring fixture + check/dry-run + receipt/manifest evidence on the target. <!-- sdd-owner: implementation -->
- [x] Walk active spec scenario sets: mark each scenario covered-by-test or explicitly hardware-gated/pending-integration-target in the evidence report; any uncovered non-hardware scenario is a task-list defect to fix before apply completes. <!-- sdd-owner: implementation -->
- [x] Final unit verification: `go test ./... && go vet ./... && go run ./tools/sync-assets --check` exit 0; transition guard exits 0; `git status --porcelain` matches the baseline plus unit-owned paths only. <!-- sdd-owner: implementation -->
