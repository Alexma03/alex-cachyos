...
| WU-15–19 | [DROPPED] Pi / Gentle AI reproduction scope | - | - | 0 | Dropped per user decision (focus exclusively on CachyOS system) |
...
## WU-15 through WU-19 — [DROPPED] Pi / Gentle AI reproduction scope

> Dropped per user decision: Pi, Gentle AI, agent models, subagents, and model routes have been entirely removed from `alex-cachyos` to avoid extraneous complexity. `alex-cachyos` focuses exclusively on reproducible CachyOS OS provisioning (Niri + Noctalia + packages + hardware/fingerprint).
...
- [ ] Tier-1 anchor test `internal/acceptance/acceptance_test.go`: one `go test ./...` entry point asserting the galaxy dry-run fixture, merge order, exact source/artifact pins, pacman name-plus-transaction policy, plan order, no-op second apply, and receipt behavior — with a traceability index mapping each of the specs' scenarios to its covering test name. Evidence: `go test ./internal/acceptance/ -v` lists the full mapping. <!-- sdd-owner: implementation -->
